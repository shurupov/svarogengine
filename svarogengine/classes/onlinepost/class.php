<?php
	...
	class OnlinePost {
		...
		static public function render_block($alias,$block_params,$template,$page_params,$face_params) {
			...
			switch ($alias) {
				case "o_form" :
					$params = self::get_form_face($block_params['p1']);
				break;
			}
			
			self::render_from_template($params,$template,$alias,$block_params["btype_class_alias"]);
		...
		static public function render_face($alias,$template,$face_params,$page_params) {
			...
			switch ($alias) {
				case "o_form_face" :
					$params = self::get_form_face($face_params['app_p1']);
				break;
			}
			
			self::render_from_template($params,$face_params["facetpl_file"],$alias,$face_params["faceclass_alias"]);
		...
		static public function render_admin_face($alias,$template,$classalias) {
			...
			switch ($alias) {
				case 'request_list' :
					$params = self::get_onlinepost_request();
				break;
				case 'delete_request' :
					$params = self::get_del_request($_GET['id']);
				break;
				case 'edit_request' :
					$params = self::get_edit_request($_GET['id']);
				break;
				case 'new_request' :
					$params = self::get_new_request();
				break;
			}
			
			self::render_from_template($params,$template,"admin",$classalias);
		...
		static public function render_admin_face_tune($facetype,$template,$faceparams,$classalias) {
			...
			switch ($facetype) {
				...
				case 'o_form_face' :
					$params = self::get_form_face_tune($facetype,$faceparams);
				break;
				
			}
			
			self::render_from_template($params,$template,'facetune',$classalias);
		...
		static public function render_admin_block_tune($blocktype,$template,$blockparams,$classalias) {
			...
			switch ($blocktype) {
				...
				case 'o_form' :
					$params = self::get_block_tune_form($blocktype,$blockparams);
				break;
				
			}
			
			self::render_from_template($params,$template,'blocktune',$classalias);
		...
		static function get_block_tune_form($blocktype,$blockparams) {
			...
			$result['forms'] = $sql_executer->get_array();
		...
		static function get_form_face_tune($facetype,$faceparams) {
			...
			$result['templates'] = self::get_face_tpl_names($facetype);
